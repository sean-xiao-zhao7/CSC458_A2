...
int _network_send(mysocket_t sd, const void *buf, size_t len)
{
    ...
    if (!ctx->is_reliable)
    {
        ...
        {
        case 0:
            printf("====>network_send:dropping the packet\n");
            /* drop the packet and forget about it. Send nothing */
            return len;

        case 1:
            /* send duplicate */
            printf("====>network_send:duplicating the packet\n");
            _network_send_packet(ctx, buf, len);
            break;

        case 2:
            /* store the packet in our queue. Will send it later */
            printf("====>network_send:keeping the packet in our queue\n");
        ...
        case 3:
            ...
            if (ctx->copied)
            {
                printf("====>network_send:sending the packet stored "
                        "in our queue\n");
